...
def require_context_adapter(f):
    ...
    def assert_adapter(self, *args, **kwargs):
        if (
            not isinstance(self._adapter, IdentityWebContextAdapter)
            or not self._adapter.has_context
        ):
            if self._logger:
                self._logger.info(
...
class IdentityWebPython(object):
    def __init__(
        ...
        adapter: IdentityWebContextAdapter = None,
        logger: Logger = None,
    ) -> None:
    ...
    def _client_factory(
        ...
        **msal_client_kwargs,
        ...
        client_config = (
            self.aad_config.client.__dict__.copy()
        )  # need to make a copy since contents must be mutated
        client_config[
            "authority"
        ] = f'{self.aad_config.client.authority}{b2c_policy or ""}'
        if token_cache:
    ...
    @require_context_adapter
    def get_auth_url(
        self, redirect_uri: str = None, b2c_policy: str = None, **msal_auth_url_kwargs
    ):
        ...
        if self.id_data.authenticated:
            auth_req_options["login_hint"] = self.id_data._id_token_claims.get(
        ...
        if self.aad_config.type.authority_type == str(AuthorityType.B2C):
            ...
            ).get_authorization_request_url(**auth_req_options)

        return self._client_factory().get_authorization_request_url(**auth_req_options)

    @require_context_adapter
    def process_auth_redirect(
        self,
        redirect_uri: str = None,
        response_type: str = None,
        afterwards_go_to_url: str = None,
    ) -> Any:
        req_params = (
            self._adapter.get_request_params_as_dict()
        )  # grab the incoming request params
        try:
            ...
            self._logger.info(
                "process_auth_redirect: no errors found in request params. continuing."
            )
            ...
            resp_type = (
                ...
                or str(ResponseType.CODE)
            )
            payload = self._extract_auth_response_payload(req_params, resp_type)
            cache = self._adapter.identity_context_data.token_cache
            redirect_uri = (
                redirect_uri or self.aad_config.auth_request.redirect_uri or None
            )

            if resp_type == str(
                ResponseType.CODE
            ):  # code request is default for msal-python if there is no response type specified
                # we should have a code. Now we must exchange the code for tokens.
                result = self._x_change_auth_code_for_token(
                    payload, cache, redirect_uri
                )
            else:
                raise NotImplementedError(
                    f"response_type {resp_type} is not yet implemented by ms_identity_web_python"
                )
            ...
            pw_reset_url = self.get_auth_url(
                redirect_uri=redirect_uri, b2c_policy=self.aad_config.b2c.password
            )
        ...
        self._logger.info(
            "process_auth_redirect: exiting auth code method. redirecting... "
        )
        return self._adapter.redirect_to_absolute_url(afterwards_go_to_url)

    @require_context_adapter
    def _x_change_auth_code_for_token(
        self, code: str, token_cache: SerializableTokenCache = None, redirect_uri=None
    ) -> dict:
        ...
        if self.aad_config.type.authority_type == str(AuthorityType.B2C):
            ...
            client = self._client_factory(
                token_cache=token_cache, b2c_policy=b2c_policy
            )
        else:
            client = self._client_factory(token_cache=token_cache)

        result = client.acquire_token_by_authorization_code(
            code, self.aad_config.auth_request.scopes, redirect_uri, id_context.nonce
        )
        return result

    @require_context_adapter
    def acquire_token_silently(
        self, scopes=None, account=None, authority=None, token_cache=None, **kwargs
    ):
    ...
    @require_context_adapter
    def _process_result(
        self, result: dict, token_cache: SerializableTokenCache
    ) -> None:
        if "error" not in result:
            ...
            if "id_token_claims" in result:
                id_context._id_token_claims = result[
                    "id_token_claims"
                ]  # TODO: if this is to stay in ctxt, use proper getter/setter
                id_context.username = id_context._id_token_claims.get(
                    "name", "anonymous"
                )
            ...
            raise TokenExchangeError(
                "_process_result: auth failed: token request resulted in error\n"
    ...
    def _parse_redirect_errors(self, req_params: dict) -> None:
        ...
        if str(AADErrorResponse.ERROR_CODE_PARAM_KEY) in req_params:
            ...
            error_code = req_params.get(
                str(AADErrorResponse.ERROR_CODE_PARAM_KEY), None
            )
            if error_code.startswith(
                str(AADErrorResponse.B2C_FORGOT_PASSWORD_ERROR_CODE)
            ):
    ...
    def _extract_auth_response_payload(
        self, req_params: dict, expected_response_type: str
    ) -> str:
        if expected_response_type == str(ResponseType.CODE):
            ...
            raise NotImplementedError(
                "Only 'code' response is currently supported by identity utils"
            )
    ...
    def sign_out(self, post_sign_out_url: str = None, username: str = None) -> Any:
        ...
        if authority_type == str(AuthorityType.B2C):
            sign_out_url = (
                f"{sign_out_url}{self.aad_config.b2c.susi}{SignOut.ENDPOINT.value}"
            )
        else:
        ...
        if post_sign_out_url:
            sign_out_url = (
                f"{sign_out_url}?{SignOut.REDIRECT_PARAM_KEY.value}={post_sign_out_url}"
            )
    ...
    ) -> None:  # TODO: complete this so it doesn't just clear the session but removes user
    ...
    @require_context_adapter
    def _generate_and_append_state_to_context_and_request(
        self, req_param_dict: dict
    ) -> str:
        state = str(uuid4())
    ...
    @require_context_adapter
    def _generate_and_append_nonce_to_context_and_request(
        self, req_param_dict: dict
    ) -> str:
        nonce = str(uuid4())
