...
class DjangoContextAdapter(IdentityWebContextAdapter):
    ...
    def identity_context_data(self) -> "IdentityContextData":
        ...
        identity_context_data = getattr(
            self.request, IdentityContextData.SESSION_KEY, None
        )
        if not identity_context_data:
            identity_context_data = (
                self._deserialize_identity_context_data_from_session()
            )
            setattr(
                self.request, IdentityContextData.SESSION_KEY, identity_context_data
            )
    ...
    def _on_request_init(self) -> None:
        try:
            idx = (
                self.identity_context_data
            )  # initialize it so it is available to request context
    ...
    def _deserialize_identity_context_data_from_session(self) -> "IdentityContextData":
        ...
        try:
            id_context_from_session = self.session.get(
                IdentityContextData.SESSION_KEY, dict()
            )
            blank_id_context_data.__dict__.update(id_context_from_session)
        except Exception as exception:
            self.logger.warning(
                f"failed to deserialize identity context from session: creating empty one\n{exception}"
            )
    ...
    def _serialize_identity_context_data_to_session(self) -> None:
        try:
            ...
            self.logger.error(
                f"failed to serialize identity context to session.\n{exception}"
            )
